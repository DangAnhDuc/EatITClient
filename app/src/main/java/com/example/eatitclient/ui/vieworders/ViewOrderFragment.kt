...
import com.example.eatitclient.Adapter.MyOrderAdapter
import com.example.eatitclient.Callback.ILoadOrderCallbackListener
...
class ViewOrderFragment : Fragment(), ILoadOrderCallbackListener {
        ...
        FirebaseDatabase.getInstance().getReference(Common.ORDER_REF)
            ...
            .addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onCancelled(p0: DatabaseError) {
                    listener.onLoadOerderFailed(p0.message)
                }
    ...
    override fun onLoadOrderSuccess(orderList: List<Order>) {
        dialog.dismiss()
        val adapter = MyOrderAdapter(context!!, orderList)
        recycler_order!!.adapter = adapter
    }

    override fun onLoadOerderFailed(message: String) {
        dialog.dismiss()
